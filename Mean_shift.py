...
def file_upload():
    # root = tk.Tk()
    z = filedialog.askopenfilename(title='Choose a file', filetypes=[("FASTA files", ".fa")])
    # root.update()
    #root.destroy()
    # root.quit()
    return z
...
tk.Label(master, text="Note: Open a FASTA File. ").grid(row=0, column=0, sticky=tk.W )
tk.Label(master, text="Note: Exit once execution is done. ").grid(row=1, column=0, sticky=tk.W )
